...
/**
 * This class do actions of schedule when it goes on
 * @author Luan Vu (hlvu.cybersoft@gmail.com)
 *
 */
public class TimeScheduleRunner extends Service implements ModeSwitcherListener {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (schedule == null) {
            Log.v("TimeScheduleRunner failed to parse the schedule from the intent");
            stopSelf();
            return START_NOT_STICKY;
        }
	...
	private void run(TimeSchedule schedule) {
        if (Log.LOGV) {
            Log.v("TimeScheduleRunner.run() " + schedule.id + " mode id: " + schedule.modeId);
        }
	...
	public void switchComplete() {
        if (Log.LOGV) {
            Log.v("TimeScheduleRunner.switchComplete()" );
        }
		safeCleanSwitcherTask();
	}
	...
	public void switchError(String errorMsg) {
        if (Log.LOGV) {
            Log.v("TimeScheduleRunner.switchError() " + errorMsg);
        }
		safeCleanSwitcherTask();
	}
