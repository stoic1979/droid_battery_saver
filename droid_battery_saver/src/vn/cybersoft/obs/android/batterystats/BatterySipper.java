...
public class BatterySipper implements Comparable<BatterySipper> {
    ...
    BatterySipper(Context context, ArrayList<BatterySipper> requestQueue,
            ...
            int iconId, Object uid, double[] values) {
        mContext = context;
    ...
    public int getUid() {
        ...
        try {
			return (Integer) uidObj.getClass().getMethod("getUid").invoke(null);
    ...
    void getQuickNameIconForUid(Object uidObj) {
        //final int uid = uidObj.getUid();
        int uid = 0;
		try {
			uid = (Integer) uidObj.getClass().getMethod("getUid").invoke(null);
		} catch (IllegalAccessException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (IllegalArgumentException e) {
     ...
     */
    public void loadNameAndIcon() throws Exception {
        // Bail out if the current sipper is not an App sipper.
        if (uidObj == null) {
            return;
        }
        PackageManager pm = mContext.getPackageManager();
        //final int uid = uidObj.getUid();
        final int uid = (Integer) uidObj.getClass().getMethod("getUid").invoke(null);
        ...
        final String uidString = Integer.toString((Integer) uidObj.getClass().getMethod("getUid").invoke(null));
